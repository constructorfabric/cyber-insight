...
def test_bitbucket_git_metrics_resolve_through_the_source_breakdown(spec: SpecRun) -> None:
    ...
    r.row("git.lines_added", "breakdown", entity_id=ERIN, dimensions=SOURCE_BITBUCKET).equals(
        value=10
    )
    r.row("git.lines_removed", "breakdown", entity_id=ERIN, dimensions=SOURCE_BITBUCKET).equals(
        value=2
    )
