...
import pytest
from insight_datapath.metric_expect import MetricResponse, Row, some
...
def test_the_later_carrier_of_an_already_authored_patch_reports_nothing(spec: SpecRun) -> None:
    ...
    assert (
        some(r.breakdown("git.commits"), dimensions={"key": "repository", "value": PATCH_COPY})
        == []
    )
    assert (
        some(r.breakdown("git.lines_added"), dimensions={"key": "repository", "value": PATCH_COPY})
        == []
    )
    assert (
        some(r.breakdown("git.commit_size"), dimensions={"key": "repository", "value": PATCH_COPY})
        == []
    )
...
def test_file_rows_under_the_repository_that_lost_the_collapse_still_count(spec: SpecRun) -> None:
    ...
    assert (
        some(r.breakdown("git.commits"), dimensions={"key": "repository", "value": XR_UPSTREAM})
        == []
    )
...
def test_merge_result_with_partly_collected_branch_commits_stays_counted(spec: SpecRun) -> None:
    ...
    by_repository(r, "git.commit_size", PARTIAL_BRANCH).equals(value=2.5)
