...
def write_service(root: Path, name: str, *, configmap: str, api_mod: str, tests: list[str]) -> None:
    ...
    for test in tests:
        (service / "src" / "api" / test).write_text("#[test]\n", encoding="utf-8")
    ...
    def test_a_service_on_the_shared_knobs_with_both_test_suites_meets_the_bar(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_service(
                ...
                tests=["log_context_tests.rs", "log_leak_tests.rs"],
            )
