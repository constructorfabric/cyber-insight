...
RULES:
  ...
  - ALWAYS give a `**Vector**` line to every requirement that makes a quality claim, functional or not, naming its one primary vector, and to every inherited entry the upstream obligation's vector; a data-correctness or access-control obligation is usually written as an FR, so restricting the tag to section 6 would leave most quality claims untagged; WHICH vectors a module needs stays advisory
