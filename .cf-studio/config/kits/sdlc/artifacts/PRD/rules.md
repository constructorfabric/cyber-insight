...
RULES:
  ...
  - ALWAYS fill a row's Show-Stopper cell with exactly one of four forms: a MUST-strength show-stopper requirement corresponding to an NFR in 6.2 that carries the obligation; `None — no show-stopper; obligations covered by {NFR IDs}` when the vector is material and has obligations in 6.2 but none of them makes the module unviable; `None — not material because {reason}` when the vector cannot break the module at all, with a reason citing a concrete scope, scale, deployment, or exclusion fact; or `None — no obligation; {vector} is material and 6.2 carries no covering NFR (gap)` when the vector is material and no obligation anywhere in 6.2 covers it
  - ALWAYS treat the `None — no obligation ... (gap)` form as a legitimate, honest entry that records a real gap for the PRD's owner to close, not a defect in the table; author it only when both claims it makes are true — the vector is material and nothing in 6.2 covers it — never as a shortcut for skipping the analysis
  ...
  - ALWAYS give every row in the 6.1 table a Rationale distinct from its Show-Stopper cell text: for a material vector — whether it carries a show-stopper requirement, is covered by 6.2 obligations without one, or has no covering obligation at all — the Rationale states the business consequence, not a restatement of the show-stopper condition, NFR list, or gap statement; for a `None — not material` row it explains why the vector is immaterial without merely repeating the reason already given in the show-stopper cell
